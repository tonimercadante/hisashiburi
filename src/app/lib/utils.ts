...
export default function getTimeData(times, total_duration) {
  ...
  while (i < times.length) {
    if (parsed_times[i]["start"] != 0 && i == 0) {
      data.push({
        percent: Math.round(
          ((parsed_times[i]["start"] - 0) / total_duration) * 100,
        ),
        inorout: false,
    ...
    data.push({
      percent: Math.round(
        ((parsed_times[i]["end"] - parsed_times[i]["start"]) / total_duration) *
          100,
      ),
      inorout: true,
      time: formatedTime(parsed_times[i]["start"], parsed_times[i]["end"]),
    ...
    if (i != times.length - 1) {
      data.push({
        percent: Math.round(
          ((parsed_times[i + 1]["start"] - parsed_times[i]["end"]) /
            total_duration) *
            100,
        ),
        inorout: false,
      ...
      data.push({
        percent: Math.round(
          ((total_duration - parsed_times[i]["end"]) / total_duration) * 100,
        ),
        inorout: false,
        time: formatedTime(parsed_times[i]["start"], parsed_times[i]["end"]),
      });
